...
async def inline_alive(o):
    MSG = "• **Masterolic Userbot •**"
    WEB0 = InputWebDocument(
        "https://telegra.ph/file/66413995a3e0221ed6b9d.jpg", 0, "image/jpg", []
    )
    RES = [
...
async def inline_handler(event):
    ...
    if INLINE_PIC:
        ...
        result = await event.builder.article(
            title="💞HANA💞 Help Menu", text=text, buttons=_main_help_menu
        )
...
async def _(e):
    button = [
        ...
        [
            Button.switch_inline(
                "Piston Eval",
                query="run javascript console.log('Hello ')",
                same_peer=True,
            ),
            Button.switch_inline(
                "OʀᴀɴɢᴇFᴏx🦊",
        ...
        [
            Button.switch_inline(
                "Tᴡɪᴛᴛᴇʀ Usᴇʀ", query="twitter Spotify", same_peer=True
            ),
            Button.switch_inline(
